...
const wchar_t * CommandLine::kProcessType = TEXT("type");
const wchar_t * CommandLine::kPipeName = TEXT("servie-pipe-token");
const wchar_t * CommandLine::kBreakpad = TEXT("client");
const wchar_t * CommandLine::kOuter = TEXT("outer");
...
static bool IsSwitch(const ATL::CString & parameter_string,
    ...
    const int32 equals_psition = parameter_string.Find(
                                     CommandLine::kDelimeter, switch_start);

    if (equals_psition == -1) {
        *switch_name = parameter_string.Mid(switch_start);
    } else {
        *switch_name = parameter_string.Mid(
                           switch_start, equals_psition - switch_start);
...
CommandLine::CommandLine(const string16 & program)
    : changed_(false) {
    if (!program.empty()) {
        program_ = program;
    }
...
const string16& CommandLine::GetCommandLine() {
    if (changed_ || argv_.empty()) {
        ...
        for (std::map<string16, string16>::const_iterator iter
                ...
                iter != switch_values_.end();
                ++iter) {
            argv.AppendFormat(TEXT(" %s%s%s\"%s\"")
                              , kPrefix
                              , iter->first.c_str()
                              , kDelimeter
                              , iter->second.c_str());
        ...
        for (std::vector<string16>::const_iterator iter
                = nonswitch_args_.begin();
                iter != nonswitch_args_.end();
                ++iter) {
            argv.AppendFormat(TEXT(" %s \"%s\"")
                              , kPrefix
                              , (*iter).c_str());
        }
...
bool CommandLine::HasExternalLink() {
    return !nonswitch_args_.empty();
}
}  // namespace base
